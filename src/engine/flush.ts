...
import { processSingleUrl } from './fetcher'
...
async function processItem(rt: ParserRuntime, session: any, platform: string, text: string, parsed: any): Promise<ProcessedItem> {
  // 各能力一律经阶段管道（基线=直通/不合并/不转换），扩展可注入或替换
  ...
  const avatar = parsed.avatar ? await runStage(rt, 'media.image', { platform, url: parsed.avatar, kind: 'avatar' }) : { kind: 'drop' as const }
  const cover = (parsed.cover && parsed.type !== 'image' && parsed.type !== 'live_photo' && parsed.type !== 'live')
    ? await runStage(rt, 'media.image', { platform, url: parsed.cover, kind: 'cover' })
    : null

  const video: VideoOutcome = parsed.video
    ? await runStage(rt, 'media.video', { platform, videoUrl: parsed.video, coverUrl: parsed.cover || '', meta: { title: parsed.title, author: parsed.author, requesterId } })
  ...
  if (parsed.isGif && video.kind === 'raw' && video.url && rt.config.gifConvertEnabled !== false) {
    gif = await runStage(rt, 'transcode', {
      url: video.url, durationSec: parsed.duration,
      opts: {
  ...
  for (const ev of parsed.extraVideos || []) {
    ...
    extraCovers.push(evCover
      ? await runStage(rt, 'media.image', { platform, url: evCover, kind: 'cover' })
      : null)
    const outcome = await runStage(rt, 'media.video', { platform, videoUrl: ev.url, coverUrl: evCover, meta: { title: parsed.title, author: parsed.author, requesterId } })
    extraVideos.push(outcome)
    if (ev.isGif && outcome.kind === 'raw' && outcome.url && rt.config.gifConvertEnabled !== false) {
      extraGifs.push(await runStage(rt, 'transcode', {
        url: outcome.url, durationSec: ev.duration || 0,
        opts: {
          maxWidth: rt.config.gifMaxWidth || 480,
          fps: rt.config.gifFps || 15,
          maxDurationSec: rt.config.gifMaxDurationSec || 15,
...
export async function flush(rt: ParserRuntime, session: SessionLike, matches: LinkMatch[], opts: FlushOptions = {}) {
  ...
  await runStage(rt, 'send', { items, session, sender, quoteId: session?.messageId })
}
